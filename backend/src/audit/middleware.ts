...
function extractContractContext(req: Request): {
  ...
  return {
    contractAddress: body.contractAddress || body.contract_address || body.contract,
    functionName: body.functionName || body.function_name || body.function || body.method,
    parameters: body.parameters || body.params || body.args || {},
    employer: body.employer || body.employerId || req.headers['x-employer-id'] as string,
  };
}
...
export function createLoggingMiddleware(auditLogger: AuditLogger) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const isContractInteraction = context.contractAddress || context.functionName;
    ...
    res.send = function (data: any) {
      ...
      if (isContractInteraction) {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          auditLogger.logContractInteraction({
            ...
            console.error('[AuditLogger] Failed to log successful interaction:', err);
          });
        } else {
          auditLogger.logContractInteraction({
            contractAddress: context.contractAddress || 'unknown',
            ...
            console.error('[AuditLogger] Failed to log failed interaction:', err);
          });
        }
      }
