...
-- Treasury monitor logs: one row per employer per monitoring cycle
CREATE TABLE IF NOT EXISTS treasury_monitor_log (
    id           BIGSERIAL   PRIMARY KEY,
    employer     TEXT        NOT NULL,
    balance      NUMERIC     NOT NULL,   -- net balance in stroops (deposits - payouts)
    liabilities  NUMERIC     NOT NULL,   -- remaining active stream obligations in stroops
    runway_days  NUMERIC,                -- NULL = infinite (no active streams draining balance)
    alert_sent   BOOLEAN     NOT NULL DEFAULT FALSE,
    checked_at   TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_monitor_log_employer    ON treasury_monitor_log (employer);
CREATE INDEX IF NOT EXISTS idx_monitor_log_checked_at  ON treasury_monitor_log (checked_at DESC);
